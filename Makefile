...
smtsynth.o: $(als_dir)/smtsynth.cc $(als_dir)/smtsynth.h
	 $(yosys-exec-command) $< -o $@ $(opt)

smt_utils.o: $(als_dir)/smt_utils.cc $(als_dir)/smt_utils.h
	$(yosys-exec-command) $< -o $@ $(opt)

yosys_utils.o: $(als_dir)/yosys_utils.cc $(als_dir)/yosys_utils.h
	$(yosys-exec-command) $< -o $@ $(opt)

als.o: $(als_dir)/als.cc
	$(yosys-exec-command) $< -o $@ $(opt)

graph.o: $(als_dir)/graph.cc $(als_dir)/graph.h
	$(yosys-exec-command) $< -o $@ $(opt)

optimizer.o: $(als_dir)/optimizer.cc $(als_dir)/optimizer.h
